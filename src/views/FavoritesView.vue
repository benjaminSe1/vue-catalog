...
const favoriteVisibleProducts = computed(() => visibleProducts.value.filter((p) => isFav(p.id)))

onMounted(async () => {
    // Ne charge les produits que si nécessaire
    if (favoritesStore.count > 0 && !loaded.value) await fetchProducts()
})
</script>
            ...
            <v-container v-else>
                ...
                <v-row v-else>
                    <v-col v-if="favoriteVisibleProducts.length === 0" cols="12">
                        <v-alert type="info" variant="tonal"> No favorites found. </v-alert>
                    </v-col>

                    <v-col v-for="item in favoriteVisibleProducts" v-else :key="item.id" cols="12" sm="6" md="6" lg="4">
