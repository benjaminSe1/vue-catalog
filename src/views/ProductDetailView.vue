...
const idProduct = computed(() => typeof route.params.id === "string" && Number.isFinite(Number(route.params.id)) ? Number(route.params.id) : null)
...
const setProductDetail = async (id: number) => {
  ...
  try {
    ...
    else error.value = { message: 'An unexpected error occurred', kind: 'network', name: 'Unexpected Error' }
  } finally {
    ...
    <v-row>
      <v-col cols="12" md="6" lg="4">
        ...
        <v-card v-else-if="currentProduct" class="product-card" variant="outlined">
          <v-btn icon variant="text" @click="toggleFavorite(currentProduct.id)" class="position-absolute top-0 right-0">
            <v-icon>
              {{ isFavorite ? 'mdi-heart' : 'mdi-heart-outline' }}
            </v-icon>
          </v-btn>
          <v-card-title>{{ currentProduct.title }}</v-card-title>
          <v-img :src="currentProduct.image" :alt="currentProduct.title" aspect-ratio="16/9" height="200" />
          <v-card-text>Price: ${{ currentProduct.price }}</v-card-text>
          <v-card-text>Rating: {{ currentProduct.rating.rate }} ({{ currentProduct.rating.count }}
            reviews)</v-card-text>
